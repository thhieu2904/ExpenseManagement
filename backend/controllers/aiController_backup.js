...
class AIController {
  ...
  parseGeminiResponse(responseText) {
    return this.utilsHelper.parseGeminiResponse(responseText);
  }
...
Response:
{
  ...
  async handleAddAccount(account, userId, responseForUser) {
    try {
      ...
      // Thực sự tạo account trong database
      const accountData = {
        name: account.name,
        type: accountType,
        bankName: bankName,
      ...
      return {
        response:
          responseForUser || `Đã tạo thành công tài khoản "${account.name}"!`,
  ...
  extractDate(text) {
    ...
    const datePatterns = [
      ...
      /vào\s*ngày\s*(\d{1,2})\/\s*(\d{1,2})/i, // vào ngày DD/MM
      /vào\s*(\d{1,2})\/\s*(\d{1,2})/i, // vào DD/MM
    ];
...
Giao dịch gần đây: ${recentTransactions
  ...
  // Helper method để parse response từ Gemini
  parseGeminiResponse(responseText) {
