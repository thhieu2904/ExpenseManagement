...
const ProfilePage = () => {
  ...
  // Import/Export State - removed as now handled by SystemSettings
  ...
  // --- SPENDING REMINDER HANDLERS ---
  const handleSpendingReminderToggle = (enabled) => {
    try {
      const success = spendingReminderService.toggleSpendingReminder(enabled);

      if (success) {
        setReminder(enabled);
        setSettingsMessage({
          text: enabled
            ? "Đã bật nhắc nhở chi tiêu. Bạn sẽ nhận thông báo hàng ngày lúc 6 PM."
            : "Đã tắt nhắc nhở chi tiêu.",
          type: "success",
        });

        // Initialize reminder service if enabled
        if (enabled) {
          spendingReminderService.initializeSpendingReminder();
          ...
          if (
            ...
            Notification.requestPermission().then((permission) => {
              if (permission === "granted") {
                setSettingsMessage({
                  text: "Nhắc nhở chi tiêu đã được bật và bạn đã cho phép thông báo trình duyệt.",
                  type: "success",
                });
              } else {
                setSettingsMessage({
                  text: "Nhắc nhở chi tiêu đã được bật. Bạn có thể cho phép thông báo trình duyệt để nhận cảnh báo.",
                  type: "info",
                });
              }
            });
        ...
        console.log(`📊 Spending reminder ${enabled ? "enabled" : "disabled"}`);
      } else {
        setSettingsMessage({
          text: "Không thể lưu cài đặt nhắc nhở. Vui lòng thử lại.",
          type: "error",
        });
      }
    } catch (error) {
      console.error("Error toggling spending reminder:", error);
      setSettingsMessage({
        text: "Có lỗi xảy ra khi cập nhật cài đặt nhắc nhở.",
        type: "error",
      });
    }
  };
